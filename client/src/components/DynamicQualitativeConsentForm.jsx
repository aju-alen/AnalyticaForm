...
import { QUALITATIVE_CONSENT_DEFAULT_ITEMS } from './QualitativeConsentForm';
...
function seedItems() {
  return QUALITATIVE_CONSENT_DEFAULT_ITEMS.map((item) => ({
    ...item,
    id: uid(5),
  }));
}
...
const DynamicQualitativeConsentForm = ({
  onSaveForm,
  data,
  id,
  disableText,
    ...
    return {
      ...
      selectedValue:
        Array.isArray(source?.selectedValue) && source.selectedValue.length === items.length
          ...
          : emptySelected(items),
      formType: 'DynamicQualitativeConsentForm',
    };
  };
  ...
  const removeItem = (index) => {
    setFormData((prev) => {
      ...
      const filteredSelected = (prev.selectedValue || []).filter((_, i) => i !== index);
      return { ...prev, items, selectedValue: syncSelected(items, filteredSelected) };
    });
  };
                ...
                {!disableText ? (
                  <Stack spacing={1}>
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={!!item.showsZoomOnYes}
                          onChange={(e) => {
                            const showsZoomOnYes = e.target.checked;
                            setFormData((prev) => ({
                              ...prev,
                              items: prev.items.map((it, i) =>
                                i === index ? { ...it, showsZoomOnYes } : it
                              ),
                            }));
                          }}
                          size="small"
...
export default DynamicQualitativeConsentForm;
