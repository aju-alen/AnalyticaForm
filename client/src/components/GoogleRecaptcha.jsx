  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const handleRecaptchaChange = (value) => {
        console.log(value,'value in recaptcha choice form');
        setRecaptchaValue(value)
        setFormData({ ...formData, selectedValue:[{
      ...
      console.log(recaptchaValue,'recaptchaValue in recaptcha choice form');
      ...
      console.log(import.meta.env.VITE_GOOGLE_RECAPTCHA_SITE_KEY,'keyyyyyyyyyyy');
  return (
    <React.Fragment>
      <CssBaseline />
  ...
  '&:hover': {
    ...
    backgroundColor:'#F4FFF8',
           ...
           <TextField fullWidth id="standard-basic" size='small' label={!disableText ? "Insert input" : ''} variant="standard" name='question' value={formData.question}
            onChange={(e) => setFormData({ ...formData, selectedValue: [{ question: formData.question, answer: '', index: '' }], question: e.target.value })}
            InputProps={{
